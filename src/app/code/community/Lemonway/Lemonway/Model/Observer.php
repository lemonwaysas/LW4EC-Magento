...
class Lemonway_Lemonway_Model_Observer {
				...
				if($order->canCancel())
				{
					try {
						...
						->addStatusToHistory($order->getStatus(),// keep order status/state
								Mage::helper('Lemonway_lemonway')->__("Order canceled automatically by cron because order is pending since %d minutes",$limitedTime));
