 ...
 */
class Lemonway_Lemonway_Model_Apikit_Apimodels_Wallet extends Varien_Object{
		...
		if(count($WALLETArr))
		{
			...
			if (isset($WALLET->DOCS))
				foreach ($WALLET->DOCS->DOC as $DOC){
					$this->_data['kyc_docs'][] = Mage::getModel('Lemonway_lemonway/apikit_apimodels_kycDoc',array($DOC));//new KycDoc($DOC);
				}
			$this->_data['ibans'] = array();
			if (isset($WALLET->IBANS))
				foreach ($WALLET->IBANS->IBAN as $IBAN){
					$this->_data['ibans'][] = Mage::getModel('Lemonway_lemonway/apikit_apimodels_iban',array($IBAN));//new Iban($IBAN);
				}
			$this->_data['ssd_mandates'] = array();
			if (isset($WALLET->SDDMANDATES))
				foreach ($WALLET->SDDMANDATES->SDDMANDATE as $SDDMANDATE){
					$this->_data['ssd_mandates'][] = Mage::getModel('Lemonway_lemonway/apikit_apimodels_ssdMandate',array($SDDMANDATE));//new SddMandate($SDDMANDATE);
				}
		}
	}
