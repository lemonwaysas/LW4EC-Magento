 ...
 */
class Lemonway_Lemonway_Adminhtml_Lemonway_WalletController extends Lemonway_Lemonway_Controller_Adminhtml_Lemonway
{
    /**
     * init the wallet
     *
    ...
    protected function _initWallet()
    {
        ...
        $wallet    = Mage::getModel('Lemonway_lemonway/wallet');
        if ($walletId) {
    ...
    {
        $this->loadLayout();
        $this->_title(Mage::helper('Lemonway_lemonway')->__('LW'))
             ->_title(Mage::helper('Lemonway_lemonway')->__('Wallets'));
        $this->renderLayout();
    }
    ...
    public function editAction()
    {
        ...
        if ($walletId && !$wallet->getId()) {
            $this->_getSession()->addError(
                Mage::helper('Lemonway_lemonway')->__('This wallet no longer exists.')
            );
            $this->_redirect('*/*/');
            return;
        }
        ...
        if ($wallet->getId()) {
            ...
            $this->_title(Mage::helper('Lemonway_lemonway')->__('Add wallet'));
        }
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost('wallet')) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('Lemonway_lemonway')->__('Wallet was successfully saved')
                );
                ...
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('Lemonway_lemonway')->__('There was a problem saving the wallet.')
                );
                Mage::getSingleton('adminhtml/session')->setWalletData($data);
                $this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id')));
                return;
        ...
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('Lemonway_lemonway')->__('Unable to find wallet to save.')
        );
        $this->_redirect('*/*/');
    }
    ...
    public function deleteAction()
    {
        if ( $this->getRequest()->getParam('id') > 0) {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('Lemonway_lemonway')->__('Wallet was successfully deleted.')
                );
                $this->_redirect('*/*/');
                return;
            } catch (Mage_Core_Exception $e) {
                ...
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('Lemonway_lemonway')->__('There was an error deleting wallet.')
                );
                $this->_redirect('*/*/edit', array('id' => $this->getRequest()->getParam('id')));
                Mage::logException($e);
                return;
        ...
        Mage::getSingleton('adminhtml/session')->addError(
            Mage::helper('Lemonway_lemonway')->__('Could not find wallet to delete.')
        );
        $this->_redirect('*/*/');
    }
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($walletIds)) {
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('Lemonway_lemonway')->__('Please select wallets to delete.')
            );
        } else {
            try {
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('Lemonway_lemonway')->__('Total of %d wallets were successfully deleted.', count($walletIds))
                );
            } catch (Mage_Core_Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError($e->getMessage());
            } catch (Exception $e) {
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('Lemonway_lemonway')->__('There was an error deleting wallets.')
                );
                Mage::logException($e);
            }
        }
    ...
    public function massStatusAction()
    {
        ...
        if (!is_array($walletIds)) {
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('Lemonway_lemonway')->__('Please select wallets.')
            );
        } else {
            try {
                foreach ($walletIds as $walletId) {
                $wallet = Mage::getSingleton('Lemonway_lemonway/wallet')->load($walletId)
                ...
                Mage::getSingleton('adminhtml/session')->addError(
                    Mage::helper('Lemonway_lemonway')->__('There was an error updating wallets.')
                );
                Mage::logException($e);
            }
        }
    ...
    {
        $fileName   = 'wallet.csv';
        $content    = $this->getLayout()->createBlock('Lemonway_lemonway/adminhtml_wallet_grid')
            ->getCsv();
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('sales/Lemonway_lemonway/wallet');
    }
}
